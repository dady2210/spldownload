...
if (args.length > 0) {
    // Si se proporciona un argumento, ejecutar fetch_m3u8 con ese valor
    //const url = args[0];
    //fetch_m3u8(url);
    const url = args[0];
    const outputPath = args[1] || "./output.ts";
    download_m3u8(url, outputPath);
} else {
    // Si no se proporcionan argumentos, imprimir un mensaje de error
    console.error("Por favor, proporciona el valor de fetch_m3u8 como argumento.");
}